...
static EventColors Calendar_EventColorForId(int colorId) {
  ...
  switch (colorId) {
  case 1:
    return (EventColors){g_theme.iris, tx}; // lavender
  case 2:
    return (EventColors){g_theme.foam, tx}; // sage
  case 3:
    return (EventColors){g_theme.love, bg}; // grape
  case 4:
    return (EventColors){g_theme.rose, tx}; // flamingo
  case 5:
    return (EventColors){g_theme.gold, tx}; // banana
  case 6:
    return (EventColors){g_theme.pine, bg}; // sage (alias)
  case 7:
    return (EventColors){// peacock
                         {(g_theme.pine.r + g_theme.foam.r) / 2,
                          (g_theme.pine.g + g_theme.foam.g) / 2,
                          (g_theme.pine.b + g_theme.foam.b) / 2, 255},
                         bg};
  case 8:
    return (EventColors){cal_hover_adjust(g_theme.iris, 30), bg}; // blueberry
  case 9:
    return (EventColors){cal_hover_adjust(g_theme.pine, 20), bg}; // basil
  case 10:
    return (EventColors){cal_hover_adjust(g_theme.love, 20), bg}; // tomato
  case 11:
    return (EventColors){cal_hover_adjust(g_theme.gold, 15), tx}; // tangerine
  default:
    return (EventColors){g_theme.pine, bg}; // default blue
  }
}
...
static float timed_event_hour(time_t t) {
  struct tm lt = *localtime(&t);
  return (float)lt.tm_hour + (float)lt.tm_min / 60.0f +
...
static void Calendar_Render(uint32_t fontId) {
  ...
  static bool selectedEventOnLeft =
      true; // true = event is left of midline, popup goes right

  // Event bucketing arrays — static so previous frame's data is available for
  // click detection
  ...
  if (menuOpen && IsMouseButtonPressed(0)) {
    if (Clay_PointerOver(
            Clay_GetElementIdWithIndex(CLAY_STRING("MenuItem"), 0))) {
      g_currentPage = PAGE_SETTINGS;
      menuOpen = false;
    }
    if (Clay_PointerOver(
            Clay_GetElementIdWithIndex(CLAY_STRING("MenuItem"), 1))) {
  ...
  if (menuOpen && IsMouseButtonPressed(0)) {
    for (int ci = 0; ci < g_calendarCount; ci++) {
      if (Clay_PointerOver(Clay_GetElementIdWithIndex(CLAY_STRING("CalRow"),
                                                      (uint32_t)ci))) {
  ...
  if (!menuOpen && selectedEvent < 0 && IsMouseButtonPressed(0)) {
    ...
    for (int i = 0; i < 7; i++) {
      for (int ei = 0; ei < colEventCount[i]; ei++) {
        ...
        Clay_ElementId eid = Clay_GetElementIdWithIndex(CLAY_STRING("TimedEvt"),
                                                        (uint32_t)evtId);
    ...
    for (int i = 0; i < 7; i++) {
      for (int ae = 0; ae < alldayEventCount[i]; ae++) {
        ...
        Clay_ElementId eid = Clay_GetElementIdWithIndex(
            CLAY_STRING("AllDayEvtClick"), (uint32_t)adId);
        if (Clay_PointerOver(eid)) {
  ...
  for (int ei = 0; ei < g_eventCount; ei++) {
    ...
    if (!g_calendars[ev->calendarIndex].visible)
      continue;
    if (ev->allDay) {
      for (int i = 0; i < 7; i++) {
        // days[i].tm_mon is 0-based; ev->startMon is 1-based
        if (allday_covers(ev, days[i].tm_year + 1900, days[i].tm_mon + 1,
                          days[i].tm_mday)) {
  ...
  if (g_currentPage == PAGE_CALENDAR) {
               ...
               .layout =
                   {
                       .sizing = {.width = CLAY_SIZING_GROW(0),
                                  .height =
                                      CLAY_SIZING_FIXED(CAL_HEADER_HEIGHT)},
                   },
               .backgroundColor = cal_cream,
        ...
        for (int i = 0; i < 7; i++) {
          ...
          CLAY(CLAY_IDI("DayHeader", i),
            ...
            CLAY_TEXT(dayName, CLAY_TEXT_CONFIG({
                                   .fontId = fontId,
                                   .fontSize = 16,
                                   .textColor = isToday[i] ? cal_accentBlue
                                                           : cal_secondaryText,
                               }));
                 ...
                 .layout =
                     {
                         .sizing = {.width = CLAY_SIZING_GROW(0),
                                    .height = CLAY_SIZING_FIXED(
                                        CAL_GRID_TOTAL_HEIGHT)},
                     },
             }) {

          // ── Time Labels Column ──
          CLAY(CLAY_ID("TimeLabels"),
               {
                   .layout =
                       {
                           .sizing = {.width =
                                          CLAY_SIZING_FIXED(CAL_GUTTER_WIDTH),
                                      .height = CLAY_SIZING_GROW(0)},
                           .layoutDirection = CLAY_TOP_TO_BOTTOM,
                       },
               }) {
          ...
          CLAY(CLAY_ID("DayColumnsArea"),
            ...
            for (int i = 0; i < 7; i++) {
              ...
              for (int ei = 0; ei < colEventCount[i]; ei++) {
                ...
                if (height < 16.0f)
                  height = 16.0f; // minimum tap target
                        ...
                        .floating =
                            {
                                ...
                                .parentId =
                                    Clay_GetElementIdWithIndex(
                                        CLAY_STRING("DayColumn"), (uint32_t)i)
                                        .id,
                                .offset = {2, yTop},
                                .zIndex = 10,
                                .pointerCaptureMode =
    ...
    if (menuOpen) {
      ...
      CLAY(CLAY_ID("MenuScrim"),
           {
               ...
               .backgroundColor = {g_theme.overlay.r, g_theme.overlay.g,
                                   g_theme.overlay.b, 160},
               .floating =
                   {
      ...
      CLAY(CLAY_ID("SidebarPanel"),
        ...
        CLAY(CLAY_ID("CalSectionHeader"),
          ...
          CLAY_TEXT(CLAY_STRING("My calendars"),
                    CLAY_TEXT_CONFIG({
                        .fontId = fontId,
                        .fontSize = 16,
                        .textColor = cal_secondaryText,
    ...
    if (selectedEvent >= 0 && selectedEvent < g_eventCount &&
        selectedEventElId != 0) {
      EventDetail(&g_events[selectedEvent], fontId, selectedEventElId,
                  selectedEventOnLeft);
    }
